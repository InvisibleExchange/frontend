...
export default class User {
  ...
  makePerpetualOrder(
    ...
    if (position_effect_type == "Open") {
      ...
      if (this.positionData[synthetic_token]) {
        perpPosition = this.positionData[synthetic_token];
  ...
  jsonStringify() {
    let noteDataJson = {};
    for (let [token, arr] in Object.entries(this.noteData)) {
      let noteArrJson = arr.map((note) => {
        note.toGrpcObject();
      });
      noteDataJson[token] = noteArrJson;
    }

    let json = {
      // userId: this.userId.toString(),
      privSpendKey: this.privSpendKey.toString(),
      privViewKey: this.privViewKey.toString(),
      // privateSeed: this.privateSeed.toString(),
      // puzbKeys
      noteCounts: this.noteCounts,
      positionCounts: this.positionCounts,
      noteData: noteDataJson,
      notePrivKeys: this.notePrivKeys,
      positionData: this.positionData,
      positionPrivKeys: this.positionPrivKeys,

      orders: this.orders,
      perpetualOrders: this.perpetualOrders,

      pfrKeys: this.pfrKeys,
      fills: this.fills,
    };
